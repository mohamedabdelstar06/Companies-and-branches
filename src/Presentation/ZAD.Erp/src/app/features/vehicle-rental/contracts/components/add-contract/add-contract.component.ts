...
export class AddContractComponent implements OnInit {
  ...
  recalculateRentPrice() {
    ...
    // Explicitly calculate dependent fields here as fallback, in case the subscription misses it
    this.calculateFields();
  }
  ...
  calculateDates() {
    ...
    switch (type) {
      ...
      case 2: // Weekly
        ...
        break;
      case 3: // Monthly (assume exactly 30 days)
        expectedDate.setDate(expectedDate.getDate() + (period * 30));
        break;
      case 4: // Long-term (Years = 365 days)
        expectedDate.setDate(expectedDate.getDate() + (period * 365));
        break;
      case 5: // Hourly
  ...
  onSubmit() {
    ...
    if (this.contractId) {
      this.contractService.update(this.contractId, dto).subscribe({
        ...
        error: (err) => this.sweetAlert.error('Error', err.error?.message || 'Error updating contract')
      });
    } else {
      this.contractService.create(dto).subscribe({
        ...
        error: (err) => this.sweetAlert.error('Error', err.error?.message || 'Error creating contract')
      });
    }
  }
